...
func (m *DBMsg) CarouselCards() []courier.CarouselCard {
	if m.Metadata_ == nil {
		return nil
	}
	...
	// Carousel cards are inside templating.carousel_cards
	templating, ok := metadata["templating"].(map[string]interface{})
	if !ok {
		return nil
	}

	carouselCardsData, ok := templating["carousel_cards"].([]interface{})
	if !ok {
		return nil
	}

	carouselCards := make([]courier.CarouselCard, len(carouselCardsData))
	for i, cardData := range carouselCardsData {
		cardMap, ok := cardData.(map[string]interface{})
		if !ok {
			continue
		}

		card := courier.CarouselCard{}

		// Parse body variables
		if bodyData, ok := cardMap["body"].([]interface{}); ok {
			card.Body = make([]string, len(bodyData))
			for j, bodyVar := range bodyData {
		...
		if buttonsData, ok := cardMap["buttons"].([]interface{}); ok {
			card.Buttons = make([]courier.CarouselCardButton, len(buttonsData))
			for j, btnData := range buttonsData {
				if btnMap, ok := btnData.(map[string]interface{}); ok {
					if subType, ok := btnMap["sub_type"].(string); ok {
						card.Buttons[j].SubType = subType
					}
					if param, ok := btnMap["parameter"].(string); ok {
						card.Buttons[j].Parameter = param
					}
				}
			}
		}

		carouselCards[i] = card
	}
